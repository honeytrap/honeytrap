FROM influx6/lxcontains-ubuntu

ADD . /go/src/github.com/honeytrap/honeytrap

WORKDIR /go/src/github.com/honeytrap/honeytrap

# Get all dependencies to fix issue with linter on ./director
# RUN go get -v

# Install gometalinter
RUN go get -u -v github.com/alecthomas/gometalinter

# Install missing lint tools
RUN gometalinter --install

# Run go tests 
RUN go test -v ./director/...
RUN go test -v ./protocol/...
RUN go test -v ./providers/...
RUN go test -v ./proxies/...
RUN go test -v ./pushers/...
RUN go test -v ./server/...
RUN go test -v ./sniffer/...
RUN go test -v ./utils/...

# Run go linters
RUN gometalinter --vendor --deadline 2m ./director/...
RUN gometalinter --vendor --deadline 2m ./protocol/...
RUN gometalinter --vendor --deadline 2m ./providers/...
RUN gometalinter --vendor --deadline 2m ./proxies/...
RUN gometalinter --vendor --deadline 2m ./pushers/...
RUN gometalinter --vendor --deadline 2m ./server/...
RUN gometalinter --vendor --deadline 2m ./sniffer/...
RUN gometalinter --vendor --deadline 2m ./utils/...
