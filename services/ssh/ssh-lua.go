...
var (
	_ = services.Register("ssh-lua", LuaShell)
)

func LuaShell(options ...services.ServicerFunc) services.Servicer {
	s, err := getStorage()
	if err != nil {
...
type sshLuaShellService struct {
	c pushers.Channel
...
func (s *sshLuaShellService) CanHandle(payload []byte) bool {
	return bytes.HasPrefix(payload, []byte("SSH"))
}

func (s *sshLuaShellService) SetChannel(c pushers.Channel) {
	s.c = c
}

func (s *sshLuaShellService) Handle(ctx context.Context, conn net.Conn) error {
	id := xid.New()
		...
		map[string]lua.LGFunction{
			"log_event": func(L *lua.LState) int {
				...
				for k, v := range params {
					opts = append(opts, event.Custom(fmt.Sprintf("ssh.%s", k), v))
				}

				s.c.Send(event.New(
					services.EventOptions,
					event.Category("ssh-lua"),
					event.Type("session"),
					event.SourceAddr(conn.RemoteAddr()),
					event.DestinationAddr(conn.LocalAddr()),
					event.Custom("ssh.sessionid", id),
					event.NewWith(opts...),
				))
			...
			for req := range requests {
				...
				func() {
					if req.Type == "shell" {
						...
						term := NewTerminal(wrappedChannel, prompt)
						...
						for {
							...
							if err := L.CallByParam(lua.P{
								...
								NRet:    1,
								Protect: true,
							}, ud, lua.LString(payload)); err != nil {
								log.Error("Error calling lua method: %s", err.Error())
								return
							}
						}
