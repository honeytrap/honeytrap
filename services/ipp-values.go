...
func (v *valStr) decode(dec decoder.Decoder) {
...
func (v *valBool) encode(buf decoder.EncoderType) {
	...
	for _, val := range v.val {
		buf.WriteUint8(v.tag)
		buf.WriteData(v.name, z)
		buf.WriteUint8(1) //lenght of bool
		if v.val {
			buf.WriteUint8(1)
		} else {
			buf.WriteUint8(0)
...
func (v *valBool) decode(dec decoder.Decoder) {
	v.name = dec.Data()
...
func (v *valRangeInt) decode(dec decoder.Decoder) {
	v.name = dec.Data()
